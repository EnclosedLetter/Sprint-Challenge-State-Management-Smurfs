...
Context API helps us share forms of data through out the whole application without having to worry so much about prop drilling.

- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do?
actions - events that trigger a state change
store - is where all state changes are handled
reducers - a function that takes in previous state and actions, and then returns the next state

Why is the store known as a 'single source of truth' in a redux application?
It is known as such because it is where all the state changes are handled so that's where you go to manage your state changes

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
State is the state of the application at the time, component state is the state of a component which is not accessible from other components unless it's explicitly passed as props to its sub components. Use Redux when there is a lot of state management because prop drilling will become a hassle.

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
Redux Thunk is a middleware that lets you call action creators that return a function instead of an action object. 

- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!
I'd say def Redux since it helps so much with props which I have a hard time with
